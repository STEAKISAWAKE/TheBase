...
#define OBJPAR_NULL(type) ((type*)0) // Makes nullifying somthing easier
...
void* open_file(const char* p_file_name, size_t* p_file_size) //Open file function to make life easier
{
    FILE* p_file;
    void* p_file_data;
    size_t size;
...
Mesh CreateMeshFromFile(const char* p_file_name)
{
    ...
    unsigned int index;
    size_t file_size;
    _MeshParsingData obj_data;
    _Mesh obj_mesh;

    p_data = open_file(p_file_name, &file_size);
